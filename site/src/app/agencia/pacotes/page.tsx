    ...
    return (
        <div>
            <div className="roteiros-header">
                ...
                <Link href="/agencia/pacote/new" className="btn-primary" style={{ padding: "10px 24px", fontSize: 14 }}>
                    + Novo Pacote
                </Link>
            </div>
            ...
            {loading ? (
                ...
                <div className="empty-state">
                    ...
                    <Link href="/agencia/pacote/new" className="btn-primary" style={{ padding: "12px 28px", fontSize: 14 }}>
                        Criar Pacote
                    </Link>
                </div>
            ) : (
                    ...
                    <table className="roteiros-table">
                        ...
                        <tbody>
                            {packages.map(pkg => (
                                <tr key={pkg.id}>
                                    <td>
                                        <Link href={`/agencia/pacote/${pkg.id}`} className="roteiro-link">
                                            {pkg.title}
                                        </Link>
                                    </td>
                                    ...
                                    <td>
                                        <span className={`status-badge ${pkg.status?.toLowerCase()}`}>
                                            {pkg.status === "ACTIVE" ? "Ativo" : pkg.status === "PAUSED" ? "Pausado" : "Arquivado"}
                                        </span>
                                    </td>
                                    <td>
                                        <div className="action-btns">
                                            <Link href={`/agencia/pacote/${pkg.id}`} className="btn-sm">Editar</Link>
