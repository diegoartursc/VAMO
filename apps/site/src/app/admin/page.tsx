...
type Tab = "overview" | "packages" | "itineraries" | "agencies" | "creators" | "clients" | "financial" | "history" | "settings";
...
export default function AdminDashboardPage() {
    ...
    const MOCK_PACKAGES: PendingPackage[] = [
    ...
    const MOCK_ITINERARIES: PendingItinerary[] = [
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (histRes.ok) { const hd = await histRes.json(); setHistory(hd); }
        } catch {
            // Full mock fallback
    ...
    const logout = () => { localStorage.removeItem("adminToken"); localStorage.removeItem("adminUser"); router.push("/admin/login"); };
            ...
            <aside style={{
                ...
                <div style={{ padding: "12px 10px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                    <button onClick={() => setTab("settings")} style={{
                        ...
                        color: tab === "settings" ? "#28C9BF" : "#64748B", cursor: "pointer",
                        fontSize: "13px", fontWeight: tab === "settings" ? "700" : "500", textAlign: "left", marginBottom: "2px",
                    }}>{Icon.settings({ size: 17, color: tab === "settings" ? "#28C9BF" : "#475569" })}<span>Configurações</span></button>
            ...
            <main style={{ flex: 1, background: "#F0F4F8", overflow: "auto" }}>
                ...
                <div style={{ padding: "24px 32px", maxWidth: "1100px" }}>
                    ...
                    {tab === "clients" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                            {[
                                { id: "t1", name: "Clara Mendes", email: "clara.mendes@gmail.com", joined: "2025-08-10", ltv: 12500, trips: 3, avatar: null },
                                { id: "t2", name: "Roberto Alves", email: "roberto.alves@hotmail.com", joined: "2025-11-20", ltv: 4800, trips: 1, avatar: null },
                                { id: "t3", name: "Mariana Souza", email: "mari.souza@yahoo.com", joined: "2026-01-05", ltv: 850, trips: 1, avatar: null },
                            ].map(client => (
                                <div key={client.id} style={{ background: "#fff", borderRadius: "20px", padding: "16px 24px", border: "1px solid rgba(226,232,240,0.7)", display: "flex", alignItems: "center", gap: "16px" }}>
                                    <div style={{ width: "40px", height: "40px", borderRadius: "50%", background: "linear-gradient(135deg, #6366F1, #4F46E5)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: "700", fontSize: "14px" }}>
                                        {client.name.charAt(0)}
                                    </div>
                                    <div style={{ flex: 1 }}>
                                        <div style={{ fontWeight: "700", fontSize: "15px", color: "#1E293B" }}>{client.name}</div>
                                        <div style={{ fontSize: "13px", color: "#64748B" }}>{client.email} · Membro desde {new Date(client.joined).toLocaleDateString("pt-BR")}</div>
                                    </div>
                                    <div style={{ textAlign: "right" }}>
                                        <div style={{ fontSize: "15px", fontWeight: "800", color: "#16A34A" }}>R$ {client.ltv.toLocaleString("pt-BR")} ltv</div>
                                        <div style={{ fontSize: "12px", color: "#94A3B8" }}>{client.trips} viagem(ns)</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {tab === "financial" && (
                        <div>
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px", marginBottom: "24px" }}>
                                <div style={{ background: "#fff", borderRadius: "18px", padding: "20px", border: "1px solid rgba(226,232,240,0.7)" }}>
                                    <div style={{ fontSize: "12px", color: "#64748B", fontWeight: "600", marginBottom: "8px" }}>Volume Bruto (GMV)</div>
                                    <div style={{ fontSize: "28px", fontWeight: "800", color: "#1E293B" }}>R$ 142.500</div>
                                </div>
                                <div style={{ background: "#fff", borderRadius: "18px", padding: "20px", border: "1px solid rgba(226,232,240,0.7)" }}>
                                    <div style={{ fontSize: "12px", color: "#64748B", fontWeight: "600", marginBottom: "8px" }}>Receita VAMO (Comissões)</div>
                                    <div style={{ fontSize: "28px", fontWeight: "800", color: "#16A34A" }}>R$ 18.250</div>
                                </div>
                                <div style={{ background: "#fff", borderRadius: "18px", padding: "20px", border: "1px solid rgba(226,232,240,0.7)" }}>
                                    <div style={{ fontSize: "12px", color: "#64748B", fontWeight: "600", marginBottom: "8px" }}>Repasses Pendentes</div>
                                    <div style={{ fontSize: "28px", fontWeight: "800", color: "#D97706" }}>R$ 4.300</div>
                                </div>
                            </div>
                            <h3 style={{ margin: "0 0 16px", fontSize: "16px", fontWeight: "700", color: "#1E293B" }}>Transações Recentes</h3>
                            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                                {[
                                    { id: "tx1", date: "2026-02-28T10:30:00Z", label: "Pacote Safari África", amount: 15000, fee: 1500, status: "PAID", agency: "Wanderlust Premium" },
                                    { id: "tx2", date: "2026-02-27T15:45:00Z", label: "Roteiro Tóquio Autêntica", amount: 4500, fee: 450, status: "PENDING_PAYOUT", creator: "Carlos Silva" },
                                    { id: "tx3", date: "2026-02-26T09:12:00Z", label: "Pacote Deserto do Atacama", amount: 7500, fee: 750, status: "PAID", agency: "Trekking Sul" },
                                ].map(tx => (
                                    <div key={tx.id} style={{ background: "#fff", borderRadius: "16px", padding: "16px 20px", border: "1px solid rgba(226,232,240,0.7)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                        <div>
                                            <div style={{ fontWeight: "700", fontSize: "14px", color: "#1E293B" }}>{tx.label}</div>
                                            <div style={{ fontSize: "12px", color: "#94A3B8", marginTop: "2px" }}>
                                                {new Date(tx.date).toLocaleString("pt-BR")} · {tx.agency || tx.creator}
                                            </div>
                                        </div>
                                        <div style={{ textAlign: "right" }}>
                                            <div style={{ fontSize: "14px", fontWeight: "700", color: "#16A34A" }}>+ R$ {tx.fee.toLocaleString("pt-BR")} <span style={{ fontSize: "11px", color: "#94A3B8", fontWeight: "500" }}>(Taxa VAMO)</span></div>
                                            <div style={{ fontSize: "12px", color: "#64748B", marginTop: "2px" }}>Total: R$ {tx.amount.toLocaleString("pt-BR")}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    {/* ═══ HISTORY TAB ═══ */}
                    {tab === "history" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: "10px", position: "relative", paddingLeft: "24px" }}>
                            {/* Vertical Line */}
                            <div style={{ position: "absolute", left: "9px", top: "20px", bottom: "20px", width: "2px", background: "#E2E8F0", zIndex: 0 }} />
                            {[...history.packages, ...history.itineraries]
                                ...
                                .map(item => (
                                    <div key={item.id} style={{
                                        ...
                                        <div style={{ position: "absolute", left: "-28px", top: "50%", transform: "translateY(-50%)", width: "12px", height: "12px", borderRadius: "50%", background: "#28C9BF", border: "3px solid #F0F4F8" }} />
                                        <div>
                                            <div style={{ fontWeight: "700", fontSize: "14px", color: "#1E293B" }}>{item.title}</div>
                                            <div style={{ fontSize: "12px", color: "#94A3B8", marginTop: "2px" }}>
                                                Submetido em {new Date(item.createdAt).toLocaleString("pt-BR")} · {item.agency?.name || item.creator?.traveler?.name}
                                            </div>
                                        </div>
                                        <span style={{
                    ...
                    {tab === "settings" && (
                        <div style={{ background: "#fff", borderRadius: "20px", padding: "32px", border: "1px solid rgba(226,232,240,0.7)", maxWidth: "600px" }}>
                            <h3 style={{ margin: "0 0 20px", fontSize: "18px", fontWeight: "800", display: "flex", alignItems: "center", gap: "8px" }}>
                                {Icon.settings({ size: 20, color: "#1FA89F" })} Configurações da Conta
                            </h3>
                            <form onSubmit={e => { e.preventDefault(); showToast("Configurações salvas (mock)!", "success"); }} style={{ display: "grid", gap: "16px" }}>
                                <div style={{ display: "grid", gap: "6px" }}>
                                    <label style={{ fontSize: "12px", fontWeight: "700", color: "#64748B" }}>Nome de Exibição</label>
                                    <input type="text" defaultValue={adminUser?.name} required style={{ padding: "12px", borderRadius: "10px", border: "1.5px solid #E2E8F0", outline: "none", fontFamily: "inherit", fontSize: "14px" }} />
                                </div>
                                <div style={{ display: "grid", gap: "6px" }}>
                                    <label style={{ fontSize: "12px", fontWeight: "700", color: "#64748B" }}>E-mail Administrativo</label>
                                    <input type="email" defaultValue={adminUser?.email} required style={{ padding: "12px", borderRadius: "10px", border: "1.5px solid #E2E8F0", outline: "none", fontFamily: "inherit", fontSize: "14px" }} />
                                </div>
                                <hr style={{ border: "0", borderTop: "1px solid #E2E8F0", margin: "4px 0" }} />
                                <div style={{ display: "grid", gap: "6px" }}>
                                    <label style={{ fontSize: "12px", fontWeight: "700", color: "#64748B" }}>Nova Senha</label>
                                    <input type="password" placeholder="Deixe em branco para não alterar" style={{ padding: "12px", borderRadius: "10px", border: "1.5px solid #E2E8F0", outline: "none", fontFamily: "inherit", fontSize: "14px" }} />
                                </div>
                                <button type="submit" style={{ marginTop: "10px", padding: "14px", borderRadius: "12px", border: "none", background: "linear-gradient(135deg, #28C9BF, #1FA89F)", color: "#fff", fontWeight: "700", cursor: "pointer", fontFamily: "inherit", fontSize: "14px" }}>
                                    Salvar Alterações
                                </button>
                            </form>
                        </div>
